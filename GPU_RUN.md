...
```
Дуэль берёт корзины из таблицы через C++ (в начале лога строка `buckets: C++ + table ...`). По замеру в облаке 200 тыс. раздач — порядка 5–10 минут на дуэль. Если дуэль идёт дольше часа, запиши, сколько она успела пройти, и не прерывай.
...
Что изменилось в коде:
- таблица с GPU передаётся в обычный тренер внутри C++, без Python-словаря;
- стратегии перед пачкой пересчитываются только для строк, изменившихся в прошлой пачке.

На этой игре плоская таблица занимает 150 млн ячеек, ~3,7 ГБ видеопамяти.
